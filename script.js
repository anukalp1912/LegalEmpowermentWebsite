...
translations.ta.listen = 'கேளுங்கள்';
translations.bn.listen = 'শুনুন';
translations.te.listen = 'వినండి';
translations.mr.listen = 'ऐका';
...
function generateLegalGuidance(userInput, category, language) {
    ...
    response += `<div class="speech-controls" aria-label="Voice guidance controls"><span class="listen-label">${t.listen}</span><button id="listenGuidanceBtn" type="button" aria-label="${speechLabels.listen}">🔊</button><button id="pauseSpeechBtn" type="button" aria-label="${speechLabels.pause}" hidden>${speechLabels.pause}</button><button id="stopSpeechBtn" type="button" aria-label="${speechLabels.stop}" hidden>${speechLabels.stop}</button><span id="speechStatus" class="speech-status" aria-live="polite">${speechLabels.ready}</span></div>`;
    response += `<div class="guidance-section" data-speech-content="understanding"><div class="guidance-section-heading"><h4>${t.summary}</h4><span class="listen-label">${t.listen}</span><button class="section-speech-button" type="button" data-speech-section="understanding" aria-label="${t.listen} ${t.summary}">🔊</button></div>`;
    ...
    response += `<div class="guidance-section" data-speech-content="rights"><div class="guidance-section-heading"><h4>${t.rights}</h4><span class="listen-label">${t.listen}</span><button class="section-speech-button" type="button" data-speech-section="rights" aria-label="${t.listen} ${t.rights}">🔊</button></div><ul style="margin-left: 20px;">`;
    ...
    getLocalizedFollowUpQuestions(category, language).forEach(question => { response += `<li style="margin-bottom: 10px;">${question} <span class="listen-label">${t.listen}</span><button class="section-speech-button question-speech-button" type="button" data-speech-text="${escapeHtml(question)}" aria-label="${t.listen}">🔊</button></li>`; });
    response += '</ul>';
    response += `<div class="guidance-section" data-speech-content="evidence"><div class="guidance-section-heading"><h4 style="margin-top: 20px;">${t.evidence}</h4><span class="listen-label">${t.listen}</span><button class="section-speech-button" type="button" data-speech-section="evidence" aria-label="${t.listen} ${t.evidence}">🔊</button></div><ul style="margin-left: 20px;">`;
    content.evidence.forEach(point => { response += `<li style="margin-bottom: 10px;">${point}</li>`; });
    response += `</ul></div><div class="guidance-section" data-speech-content="roadmap"><div class="guidance-section-heading"><h4 style="margin-top: 20px;">${t.roadmap}</h4><span class="listen-label">${t.listen}</span><button class="section-speech-button" type="button" data-speech-section="roadmap" aria-label="${t.listen} ${t.roadmap}">🔊</button></div><ol style="margin-left: 20px;">`;
